...
- All container lifecycle work goes through `bantora-docker.sh`. Never call `docker`, `docker-compose`, or Gradle directly.
...
- Improve automation by enhancing `bantora-docker.sh`; do not add parallel helper scripts.
...
- Tests cover new logic and pass via `bantora-docker.sh`.
- Docker workflows operate exclusively through `bantora-docker.sh` with validated health checks and logs.
...
- Direct Docker/Gradle commands, bypassing `bantora-docker.sh`.
