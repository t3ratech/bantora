...
- Target modules impacted (e.g., `bantora-api`, `bantora-web`, `bantora-telegram`, `bantora-common-*`).
...
4. Add automated tests:
   ...
   - Patrol 4.0 UI tests for Flutter web SPA flows
...
- Run:
  ...
  - If not healthy, `./bantora-docker.sh -rrr bantora-database bantora-api bantora-web bantora-telegram`

3) Implement shared contracts (if needed)
- Edit `bantora-common/bantora-common-shared/` to add/update DTOs, enums, error shapes.
- Follow package conventions in `com.t3ratech.bantora.common.shared.dto`.
- Add tests in `bantora-common-shared` for DTO mappers or utility logic.
...
- Tests:
  - Add JUnit tests under `bantora-api/src/test/java/...` for services and controllers (WebMvcTest/MockMvc where applicable).
...
- Tests:
  - Add Patrol tests in `bantora-web/bantora_app/patrol_test/` covering the new flow (registration/login if needed, navigation, and assertions).
  - Execute via `./bantora-docker.sh --test patrol`.

6) Wire Telegram (if applicable)
- In `bantora-telegram`:
  - Update `TelegramBotService` to parse and route any new commands to `BantoraApiClient`.
  - Ensure `TelegramProperties` has correct keys set in env/properties:
    - `bantora.telegram.bot.token`
    - `bantora.telegram.bot.username`
    - `bantora.telegram.bot.api-base-url` (must include `/api` and use hyphen, not underscore)
...
- Run (adjust list to affected modules):
  ...
  - `./bantora-docker.sh -rrr bantora-web`
  - `./bantora-docker.sh -rrr bantora-telegram`
- Verify:
  - `./bantora-docker.sh --status` (all healthy)

8a) Start test environment and run comprehensive multi-language tests
// turbo
- Start test environment:
  - `./bantora-docker.sh --test-env start`
- Verify test environment health:
  - `./bantora-docker.sh --test-env status`
- Run multi-language test suite:
  - `./bantora-docker.sh --test-multi-lang unit` — Unit tests in all languages
  - `./bantora-docker.sh --test-multi-lang integration` — API integration tests in all languages
  - `./bantora-docker.sh --test-multi-lang ui` — Patrol UI tests in all languages
- Stop test environment when done:
  - `./bantora-docker.sh --test-env stop`
...
- Run (sample flow, adapt per feature):
  ...
  - Register unique user:
    - `USER="user$(date +%s)"; EMAIL="$USER@bantora.local"; PASS="Reg1!$RANDOM"`
    - `curl -s -i -H 'Content-Type: application/json' -d "{\"username\":\"$USER\",\"email\":\"$EMAIL\",\"password\":\"$PASS\"}" http://localhost:8081/api/v1/auth/register`
  - Login and capture cookie:
    - `curl -s -i -c /tmp/bantora_cookies.txt -H 'Content-Type: application/json' -d "{\"username\":\"$USER\",\"password\":\"$PASS\"}" http://localhost:8081/api/v1/auth/login`
  - Exercise new API endpoints with `-b /tmp/bantora_cookies.txt` and assert JSON conditions with `jq` where appropriate.

10) Patrol UI tests
- Create tests under `bantora-web/bantora_app/patrol_test/`:
...
- Run:
  - `./bantora-docker.sh --status`
  - Re-run a subset of cURL and Patrol UI tests to validate after rebase/merge.
...
- Add JPA/persistence to `bantora-telegram` as required.
- For `bantora-web`, all runtime values must come from `/ui/config` with env-backed values; missing envs must fail.
