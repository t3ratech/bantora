---
description: End-to-end Bantora feature implementation from ARCHITECTURE.md section with builds, health checks, curl + Playwright + unit tests, and docs updates
...
- Tests:
  - Add Playwright tests in `bantora-web/tests/` covering the new flow (registration/login if needed, navigation, command send, state assertions).
...
- Run multi-language test suite:
  ...
  - `./bantora-docker.sh --test-multi-lang ui` — Playwright UI tests in all languages
...
- Pre-req (once):
  - `npm init -y` (in `bantora-web` or repo root you choose to host tests)
  - `npm i -D @playwright/test` and `npx playwright install --with-deps`
- Create tests under `bantora-web/tests/`:
