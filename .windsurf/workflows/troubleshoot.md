...
- If issue involves user-facing features, run multi-language tests to validate:
  - `./bantora-docker.sh --test-env start` - Start test environment
  - `./bantora-docker.sh --test-multi-lang unit` - Test unit layer across languages
  - `./bantora-docker.sh --test-multi-lang integration` - Test API layer across languages
  - `./bantora-docker.sh --test-multi-lang ui` - Test UI layer across languages
  - `./bantora-docker.sh --test-env stop` - Stop test environment
