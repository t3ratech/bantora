...
3. We must also understand the build system, so read docker-composer.yml, .env and bantora-docker.sh for understanding
4. We must start the build on a clean sheet, so we must use bantora-docker.sh to destroy all module docker instances
5. The aim is not only to bring up the system (with all tests passing), but it is to bring up the system REALIABLY, PREDICTABLY, CONSISTENTLY every time using bantora-docker.sh, and it means all steps must be validated very very in depth.
